...
/**
 * 1.1. Describe Java inner classes and develop the code that uses Java inner
 * classes (such as: nested class, static class, local class and anonymous
 * classes) 1.2. Define and write functional interfaces 1.3. Describe a Lambda
 * expression; refactor the code that use anonymous inner class to use Lambda
 * expression; including type inference, target typing 2.1. Describe the built
 * in interfaces included in Java 8 – java.uilt.function package 2.2. Develop
 * code that uses Function interface 2.3. Develop code that uses Consumer
 * interface 2.4. Develop code that uses Supplier interface 2.5. Develop code
 * that uses UnaryOperator interface 2.6. Develop code that uses Predicate
 * interface 2.7. Develop the code that use primitive and binary variations of
 * base interfaces of java.util.function package 2.8. Develop the code that use
 * method reference; including refactor the code that use Lambda expression to
 * use method references 3.1. Develop the code that iterates a collection by
 * using forEach; including method chaining 3.2. Describe the Stream interface
 * and pipelines 3.3. Filter a collection using lambda expressions 3.4. Identify
 * lambda operations that are lazy 4.1. Develop the code to extract data from an
 * object using map 4.2. Search for data using search methods including:
 * findFirst, findAny, anyMatch, allMatch, noneMatch. 4.4. Perform calculations
 * using methods: count, max, min, average, sum 4.5. Sort a collection using
 * lambda expressions 4.6. Save results to a collection by using the collect
 * method and Collector class; including methods such as averagingDouble,
 * groupingBy, joining, partitioningBy 5.1. Develop the code that use parallel
 * streams -- hm... 5.2. Implement decomposition, reduction, in streams 6.4.
...
public class Lambdas {
	...
	/**
	 * 1.1. Describe Java inner classes and develop the code that uses Java
	 * inner classes (such as: nested class, static class, local class and
	 * anonymous classes) 1.2. Define and write functional interfaces 1.3.
	 * Describe a Lambda expression; refactor the code that use anonymous inner
	 * class to use Lambda expression; including type inference, target typing
	 * 2.8. Develop the code that use method reference; including refactor the
	 * code that use Lambda expression to use method references
	 */
	@Test
	public void fileFilter() {
		...
		// vs lambda
		new File(".").listFiles(File::isHidden); // inference if the parameters
													// (a File in this case) and
	 ...
	 * java.uilt.function package 2.2. Develop code that uses Function interface
	 * 2.3. Develop code that uses Consumer interface 2.4. Develop code that
	 * uses Supplier interface 2.5. Develop code that uses UnaryOperator
	 * interface 2.6. Develop code that uses Predicate interface 2.7. Develop
	 * the code that use primitive and binary variations of base interfaces of
	...
	public void javaUtilFunctionPackage() {
		...
		java.util.function.Consumer<String> chainOfConsumersForSameValue = consumer.andThen((s) -> {
		}).andThen((s) -> {
		});
		...
		Runnable r = () -> System.out.println("a"); // WORKS with STATEMENT -
													// cause it returns void

		Consumer<String> c = (str) -> new ArrayList().add(str); // add function
																// return
																// boolean so
																// this is a
																// statement -
																...
																// even without
																// {}
		// Consumer<String> s = (str)->""; // incompatible return type string
		// now - WTF ! Error:(303, 37) java: incompatible types: bad return type
		// in lambda expression java.lang.String cannot be converted to void

		// can use local vars in lambada but they have to be EFFECTIVELLY FINAL
		String aLocalVar = "a";
		Runnable runnbale = () -> {
			System.out.println(aLocalVar);
		}; // no changes to aLocalVar after this usage will compile -
			// COMPILATION FAILURE HERE
			// CAN access an instance var even if that one is not final
			// aLocalVar = "b"; // this can cause compilation failure in the
			// lambda
			// above LINE
			//
	...
	/*
	 * CAN ONLY BE CONSUMED ONCE stream can only have one termianl operation -
	 * after that - exception: java.lang.IllegalStateException
	 *
	 */
	...
	public void methodReferences() {
		...
		dishes.sort(Comparator.comparing(Dish::getName)); // so this is a
															// shorthand for a
															// lamba wih a
															// single method
															// call
		...
		// reference to an instance - will translate to a function that receive
		// the instance and executes on it
		// (instanceRef, params)->intanceRef.instanceMethod(params);
		// notice that the function receives as param the instance object upon
		// wich to execute and the
		...
		// reference to an instance method of and existing object - only the
		// parameter of the function is included in the lambda declaration,
		// (params)->existingObject.instanceMethod(params);
		Consumer<String> c1 = System.out::println;
		Consumer<String> c2 = (String str) -> System.out.println(str);

		Runnable r1 = Thread.currentThread()::getName; // Error:(324, 43) java:
														// invalid method
														// reference static
														// bound method
														// reference
		...
		r3 = () -> {
			this.t();
			assertTrue(Lambdas.this == this);
		};
		Runnable r4 = super::notify;
		r4 = () -> {
			Lambdas.super.notify();
		};
		r4 = () -> {
			super.notify();
			assertTrue(Lambdas.super.hashCode() == super.hashCode());
		};
	...
	/*
	 * a FUNCTIONAL INTERFACE has ONLY ONE ABSTRACT METHOD- more than that and
	...
	/**
	 * 3.1. Develop the code that iterates a collection by using forEach;
	 * including method chaining
	 */

	@Test
	public void t1() {
		List<String> list = dishes.stream().filter((d) -> d.calories > 100).map(Dish::getName).limit(3)
	...
	@Test
	public void t2() {
		List<Dish> list = dishes.stream().filter(Dish::isVegetarian).collect(Collectors.toList());
		out.println(list);
	}
	...
	@Test
	public void flatMap() {
		Arrays.asList("abc", "efg").stream().map((s) -> {
			return s.split("\\W*");
	...
	@Test
	public void filter() {
		List<Transaction> list = transactions.stream().filter((t) -> t.year == 2011)
				.sorted(Comparator.comparingInt(Transaction::getValue)).collect(Collectors.toList());

		List<String> cities = transactions.stream().map(t -> t.trader.city).distinct().collect(Collectors.toList());
		System.out.println(cities);

		List<Trader> traders = transactions.stream().map(t -> t.trader)
				.filter(t -> "Cambridge".equalsIgnoreCase(t.city)).sorted(Comparator.comparing(Trader::getName))
				.collect(Collectors.toList());
		System.out.println(traders);

		List<String> traderNames = transactions.stream().map(t -> t.trader.getName()).sorted()
				.collect(Collectors.toList());

		List<Integer> transactionValue = transactions.stream().filter(t -> "Cambridge".equalsIgnoreCase(t.trader.city))
	...
	public void combineTwoStreamAllCombinations() {
		...
		List<int[]> combinations = list1.stream().flatMap(i -> list2.stream().map(j -> new int[] { i, j }))
				.collect(Collectors.toList());
		combinations.stream().map(Arrays::toString).forEach(System.out::println);
	}

	/*
	 * 4.2. Search for data using search methods including: findFirst, findAny,
	...
	public void findAny() {
		...
		transactions.stream().filter(t -> "Milan".equalsIgnoreCase(t.trader.city)).findAny()
	...
	public void reduceSumProdMinMaxCount() {
		...
		Integer minTransactionValue = transactions.stream().map(Transaction::getValue).min(Integer::compare).get();

		// MAX
		list1.stream().reduce(Integer::max);
		Integer maxTransactionValue = transactions.stream().map(Transaction::getValue).max(Integer::compare).get();

		maxTransactionValue = transactions.stream().map(Transaction::getValue).reduce(0, Integer::max);

		maxTransactionValue = transactions.stream().map(Transaction::getValue).reduce(0, (a, b) -> a > b ? a : b);
	...
	/*
	 * 4.6. Save results to a collection by using the collect method and
	 * Collector class; including methods such as averagingDouble, groupingBy,
	 * joining, partitioningBy 5.2. Implement decomposition, reduction, in
	 * streams
	 */

	/**
	 * counting summingInt averagingInt IntSummaryStatistics
	 * summarizingInt(mapping function) maxBy(comparingInt) minBy(comparingInt)
	 * Collectors.joining
	 */
	@Test
	...
	public void grouping() {
		...
		Map<Dish.Type, Long> a = dishes.stream().collect(Collectors.groupingBy(Dish::getType, Collectors.counting()));

		Map<Dish.Type, Optional<Dish>> max = dishes.stream().collect(
		...
		Map<Dish.Type, Dish> max2 = dishes.stream().collect(Collectors.groupingBy(Dish::getType, Collectors
		...
		// result is a mapping
		Map<Dish.Type, Set<String>> dishMapping = dishes.stream()
				.collect(Collectors.groupingBy(Dish::getType, Collectors.mapping(f, Collectors.toSet())));
	...
	public void partitioning() {
		...
		Map<Boolean, Map<Dish.Type, List<Dish>>> l = dishes.stream()
				.collect(Collectors.partitioningBy(Dish::isVegetarian, Collectors.groupingBy(Dish::getType)));

		Map<Boolean, Map<Boolean, List<Dish>>> partition = dishes.stream().collect(Collectors
				.partitioningBy(Dish::isVegetarian, Collectors.partitioningBy((Dish d) -> d.getCalories() > 500)));

		Map<Boolean, Long> map2 = dishes.stream()
	...
	public void ownCollector() {
		...
		dishes.stream().collect(ArrayList::new, List::add, List::addAll); // Function.identity(),
																			// Characteristics:
																			// IDENTITY_FINISH,
																			// CONCURRENT
	...
	/**
	 * mapToInt IntStream: average, min, max, sum, count
	 * <p>
	 * findAny, findFirst - return Optional min, max return Optional
	 * <p>
	...
	public void streamsOfNumbers() {
		...
		IntStream.of(1, 2, 3, 4, 5, 6, 7, 8, 9).sorted(); // NO COMPARATOR
															// NEEDED FOR
															// PRIMITIVES!
		...
		// mark stream for sequential or paralel processing - ONLY LAST FLAG
		// WINS
	...
	public void pythagoreanNumbers() {
		...
		}); // valid only for consecutive pythgorean numbers - some are not at
			// all

		Stream<int[]> pythagoreanNumbersStream = IntStream.rangeClosed(1, 100).boxed()
		...
		IntStream.rangeClosed(1, 100).boxed().flatMap(
				a -> IntStream.rangeClosed(a, 100).mapToObj(b -> new int[] { a, b, (int) Math.sqrt(a * a + b * b) }))
				.filter(i -> i[2] % 1 == 0);
...
class B extends A {
	...
	@Test
	public void streamApiInFiles() throws Exception {
		// Files.walk - return Stream<Path>
		Files.walk(Paths.get("c:\\"), FileVisitOption.FOLLOW_LINKS).forEach(System.out::println);

		// Files.find(Path path, int depth, BiPreedicate<BasicFileAttributes>,
		// FileVisitOption)
		String desktop = "C:\\Users\\vasilei\\Desktop";
		Files.find(Paths.get(desktop), 0, (path, attrs) -> {
			return attrs.isRegularFile() && path.getFileName().endsWith("lnk");
		} , FileVisitOption.FOLLOW_LINKS).forEach(System.out::println);
		;
			

		// java 7 way
		try (DirectoryStream<Path> java7DirectoryStream = Files.newDirectoryStream(Paths.get(desktop))) {

			for (Path path : java7DirectoryStream) {
				System.out.println(path);
			}

			Iterator<Path> it = java7DirectoryStream.iterator();
			while (it.hasNext()) {
				System.out.println(it.next());
			}

			// MUST CLOSE THIS STREAM
			// java7DirectoryStream.close();
